...
function prepareLowRankNeighborhoodData{T<:FloatingPoint}(
  ...
  D = getNeighborhoodD(X, nodeInd, NodeBasis, EdgeBasis, param)
  E = getNeighborhoodE(X, nodeInd, NodeBasis, EdgeBasis, param)
...
function estimate_neighborhood{T<:FloatingPoint}(
  ...
  #
  θ, A11, A12, A22, b1, b2, groups = prepareLowRankNeighborhoodData(X, nodeInd, NodeBasis, EdgeBasis, param; min_eigval=min_eigval)
  θ1 = sub(θ, 1:K)
